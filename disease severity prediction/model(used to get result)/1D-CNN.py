...
for Method in range(2, 4):
    ...
    for featureNumber in range(5, 11):
        ...
        if not os.path.exists(f"../result/numpy data/1D-CNN/{method}"):
            os.makedirs(f"../result/numpy data/1D-CNN/{method}")
        ...
        if flag:
            sys.stdout = open(f"../result/model result(final)/1D-CNN.txt", "w")
            flag = False
        else:
            sys.stdout = open(f"../result/model result(final)/1D-CNN.txt", "a")
        print("=" * 60 + method + "=" * 60)
        print("=" * 40 + f"特征选择数量:{featureNumber}" + "=" * 40)
        print("=" * 30 + "特征" + "=" * 30)
        print(feature)
