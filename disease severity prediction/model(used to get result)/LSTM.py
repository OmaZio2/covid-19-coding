...
for Method in range(2, 4):
    ...
    for featureNumber in range(5, 11):
        ...
        dir_name = "../pth(final)/" + "LSTM/" + method + f"/feature_number{featureNumber}"
        ...
        if flag:
            sys.stdout = open(f"../result/model result(final)/LSTM.txt", "w")
            flag = False
        else:
            sys.stdout = open(f"../result/model result(final)/LSTM.txt", "a")
        ...
        if not os.path.exists(f"../result/numpy data/LSTM/{method}"):
            os.makedirs(f"../result/numpy data/LSTM/{method}")

        np.save(f"../result/numpy data/LSTM/{method}/LSTM(Y_true){featureNumber}.npy", Y_true)
        np.save(f"../result/numpy data/LSTM/{method}/LSTM(Y_predict_probability){featureNumber}.npy", Y_predict_probability)
