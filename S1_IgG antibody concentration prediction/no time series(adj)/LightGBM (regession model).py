...
for Method in range(1, 4):
    ...
    for index in range(0, 5):
        ...
        LGBM_Model = LGBMRegressor(n_estimators=600, learning_rate=0.01, boosting_type='gbdt', objective='regression',
                                   metric='mae', eval_metric='mae',
                                   num_leaves=60, colsample_bytree=0.7, feature_fraction=0.7, min_data=100,
                                   min_hessian=1)
        LGBM_Model.fit(X_train, Y_train, eval_set=(X_test, Y_test), verbose=True)
        part_predict = LGBM_Model.predict(X_test)
    ...
    if flag:
        sys.stdout = open(f"../result/model result(adjust)/LightGBM/{method}_LightGBM.txt", "w")
        flag = False
    else:
        sys.stdout = open(f"../result/model result(adjust)/LightGBM/{method}_LightGBM.txt", "a")
    ...
    plt.savefig(f"../result/model result(adjust)/LightGBM/{method}_LightGBM.png", dpi=600)
    # plt.show()
