...
for Method in range(1, 4):
    ...
    for index in range(0, 5):
        ...
        dir_name = "../pth(adjust)/" + "LSTM/" + method
    ...
    if flag:
        sys.stdout = open(f"../result/model result(adjust)/LSTM/{method}_LSTM.txt", "w")
        flag = False
    else:
        sys.stdout = open(f"../result/model result(adjust)/LSTM/{method}_LSTM.txt", "a")
    ...
    plt.savefig(f"../result/model result(adjust)/LSTM/{method}_LSTM.png", dpi=600)
    # plt.show()
