...
for Method in range(1, 4):
    ...
    if flag:
        sys.stdout = open(f"../result/model result(final)/LightGBM/{method}_LightGBM.txt", "w")
        flag = False
    else:
        sys.stdout = open(f"../result/model result(final)/LightGBM/{method}_LightGBM.txt", "a")
    ...
    plt.savefig(f"../result/model result(final)/LightGBM/{method}_LightGBM.png", dpi=600)
    # plt.show()
