...
for Method in range(1, 4):
    ...
    if flag:
        sys.stdout = open(f"../result/model result(final)/CatBoost/{method}_CatBoost.txt", "w")
        flag = False
    else:
        sys.stdout = open(f"../result/model result(final)/CatBoost/{method}_CatBoost.txt", "a")
    ...
    plt.savefig(f"../result/model result(final)/CatBoost/{method}_CatBoost.png", dpi=600)
    # plt.show()
