...
# sys.stdout = open('D:\Code\covid-19\COVID-19-Prediction-master\COVID-19 Prediction Code\disease severity prediction\\result\\2D-CNN.txt', 'a')
...
for Method in range(2, 4):
    ...
    for featureNumber in range(5, 11):
        ...
        if flag:
            sys.stdout = open(f"../result/model result(final)/2D-CNN(new).txt", "w")
            flag = False
        else:
            sys.stdout = open(f"../result/model result(final)/2D-CNN(new).txt", "a")
