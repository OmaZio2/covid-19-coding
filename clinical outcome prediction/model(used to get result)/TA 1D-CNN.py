...
for Method in range(2, 4):
    ...
    for featureNumber in range(5, 11):
        ...
        # 指定用于保存最优模型的路径
        dir_name = "../pth(final)/" + "TA_1D-CNN/" + method + f"/feature_number{featureNumber}"
        # 如果没有这个文件夹则创建
        ...
        np.save(f"../result/numpy data/TA_1DCNN/{Method} TA_1D-CNN(Y_true){featureNumber}.npy", Y_true)
        np.save(f"../result/numpy data/TA_1DCNN/{Method} TA_1D-CNN(Y_predict_probability){featureNumber}.npy", Y_predict_probability)

        # 将输出重定向到指定文件，用于保存相关数据
        if flag:
            sys.stdout = open(f"../result/model result(final)/TA_1D-CNN(best is 7).txt", "w")
            flag = False
        else:
            sys.stdout = open(f"../result/model result(final)/TA_1D-CNN(best is 7).txt", "a")
        print("=" * 60 + method + "=" * 60)
        print("=" * 40 + f"特征选择数量:{featureNumber}" + "=" * 40)
        print("=" * 30 + "特征" + "=" * 30)
        print(feature)
        print('TA_1D-CNN (clinical outcome remove ICU and Day):')
