...
for Method in range(2, 4):
    ...
    for featureNumber in range(5, 11):
        ...
        for index in range(0, 5):
            ...
            from keras.callbacks import ReduceLROnPlateau, ModelCheckpoint
            # 指定用于保存最优模型的路径
            dir_name = "../pth(adjust)/"+"ta_LSTM/"+method+f"/feature_number{featureNumber}"
            # 如果没有这个文件夹则创建
            ...
            TA_OneCNN_model.fit(X_train, Y_train, epochs=20, batch_size=128,
        ...
        if flag:
            sys.stdout = open(f"../result/model result(adjust)/TA_LSTM(best is 8).txt", "w")
            flag = False
        else:
            sys.stdout = open(f"../result/model result(adjust)/TA_LSTM(best is 8).txt", "a")
        ...
        print('TA_LSTM:', confusion_matrix2)
