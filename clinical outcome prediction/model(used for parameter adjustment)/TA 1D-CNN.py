...
train_path = 'C:\\Users\\Win\\Desktop\\dataset\\data\\Pre-processed outcome and severity dataset (with Time)\\82_train\\train\\oas_train%d.xlsx'
test_path = 'C:\\Users\\Win\\Desktop\\dataset\\data\\Pre-processed outcome and severity dataset (with Time)\\82_train\\test\\oas_test%d.xlsx'
df_train_list = []
df_test_list = []
...
for Method in range(2, 4):
    method = featureSelectionMethod2(Method)
    print("="*60 + method + "="*60)
    for featureNumber in range(5, 11):
        ...
        for index in range(0, 5):
            ...
            # 指定用于保存最优模型的路径
            dir_name = "../pth(adjust)/"+"ta_1D-CNN/"+method+f"/feature_number{featureNumber}"
            # 如果没有这个文件夹则创建
            ...
            # 构建模型
            TA_OneCNN_model = build_model()
            TA_OneCNN_model.fit(X_train, Y_train, epochs=30, batch_size=128,
                             validation_data=(X_test, Y_test), class_weight=class_weights
                             , verbose=2, callbacks=[checkpoint])
            # 得出准确率和loss
            loss, accuracy = TA_OneCNN_model.evaluate(X_test, Y_test, verbose=2)
            # 预测概率类别
            part_predict = TA_OneCNN_model.predict_classes(X_test)
            ...
            part_predict_probability = TA_OneCNN_model.predict(X_test).ravel()
            Y_predict_probability.extend(part_predict_probability)
        Y_true = np.array(Y_true)
        Y_predict = np.array(Y_predict)

        if flag:
            sys.stdout = open(f"../result/model result(adjust)/1D-CNN (best is 10).txt", "w")
            flag = False
        else:
            sys.stdout = open(f"../result/model result(adjust)/1D-CNN (best is 10).txt", "a")
        ...
        print('TA_1D-CNN:', confusion_matrix2)
