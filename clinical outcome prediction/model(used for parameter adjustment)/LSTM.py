...
for Method in range(2, 4):
    ...
    for featureNumber in range(5, 11):
        ...
        for index in range(0, 5):
            ...
            class_weights = class_weight.compute_class_weight('balanced', classes=np.unique(all_y), y=all_y)
            ...
            LSTM_model.fit(X_train, Y_train, epochs=20, batch_size=128,
        ...
        if flag:
            sys.stdout = open(f"../result/model result(adjust)/LSTM(best is 9).txt", "w")
            flag = False
        else:
            sys.stdout = open(f"../result/model result(adjust)/LSTM(best is 9).txt", "a")
