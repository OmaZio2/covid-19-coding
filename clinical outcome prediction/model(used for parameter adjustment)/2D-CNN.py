...
for Method in range(2, 4):
    ...
    for featureNumber in range(5, 11):
        ...
        for index in range(0, 5):
            ...
            dir_name = "../pth(adjust)/" + "2D-CNN/" + method + f"/feature_number{featureNumber}"
            # 如果没有这个文件夹则创建
            if not os.path.exists(dir_name):
                os.makedirs(dir_name)
            # 补充完整路径
            pth_path = dir_name + f"/train{index + 1}.tfl"
        ...
        if flag:
            sys.stdout = open(f"../result/model result(adjust)/2D-CNN.txt", "w")
            flag = False
        else:
            sys.stdout = open(f"../result/model result(adjust)/2D-CNN.txt", "a")
